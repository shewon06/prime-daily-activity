...
class FirebaseDataRepository(
    ...
    override suspend fun registerStaff(profile: StaffProfile): Result<Unit> = runCatching {
        require(profile.salesCode.isNotBlank())
        val ref = db.collection("staff").document(profile.salesCode)
        require(!ref.get().await().exists()) { "Sales Code already registered" }
        ref.set(profile.copy(approvalStatus = ApprovalStatus.PENDING).toMap()).await()
    }

    override suspend fun getStaffBySalesCode(salesCode: String): Result<StaffProfile?> = runCatching {
        db.collection("staff").document(salesCode).get().await().toStaffProfile()
    }

    override suspend fun getPendingRegistrations(): Result<List<StaffProfile>> = runCatching {
        db.collection("staff").whereEqualTo("approvalStatus", ApprovalStatus.PENDING.name)
            .get().await().documents.mapNotNull { it.toStaffProfile() }
    }

    override suspend fun setApproval(salesCode: String, status: ApprovalStatus): Result<Unit> = runCatching {
        db.collection("staff").document(salesCode).update("approvalStatus", status.name).await()
    }
    ...
    override suspend fun getTodayActivity(salesCode: String): Result<DailyActivity?> = runCatching {
        ...
        (doc.get("activity") as? Map<String, Any?>)?.toDailyActivity()
    }
    ...
    override suspend fun saveTodayActivity(salesCode: String, activity: DailyActivity): Result<Unit> = runCatching {
        ...
        db.runTransaction { tx ->
            val old = tx.get(ref)
            @Suppress("UNCHECKED_CAST")
            val oldActivity = (old.get("activity") as? Map<String, Any?>)?.toDailyActivity()
            if (oldActivity?.dayLocked == true) error("Day is already locked")
            if (oldActivity?.planLocked == true) {
                require(activity.prospectingPlan == oldActivity.prospectingPlan)
                require(activity.followUpsPlan == oldActivity.followUpsPlan)
                require(activity.appointmentsPlan == oldActivity.appointmentsPlan)
                require(activity.presentationsPlan == oldActivity.presentationsPlan)
            }
            tx.set(ref, mapOf("salesCode" to salesCode, "date" to todayKey(), "activity" to activity.toMap()), com.google.firebase.firestore.SetOptions.merge())
        }.await()
    }
...
private fun com.google.firebase.firestore.DocumentSnapshot.toStaffProfile(): StaffProfile? {
    ...
    return StaffProfile(getString("salesCode") ?: id, getString("fullName") ?: "", getString("mobile") ?: "", getString("zone") ?: "",
        runCatching { UserRole.valueOf(getString("role") ?: UserRole.STAFF.name) }.getOrDefault(UserRole.STAFF), getString("photoUri"),
...
private fun Map<String, Any?>.toDailyActivity() = DailyActivity(
    (get("prospectingPlan") as? Number)?.toInt() ?: 0, (get("followUpsPlan") as? Number)?.toInt() ?: 0,
    (get("appointmentsPlan") as? Number)?.toInt() ?: 0, (get("presentationsPlan") as? Number)?.toInt() ?: 0,
    (get("prospectingDone") as? Number)?.toInt() ?: 0, (get("followUpsDone") as? Number)?.toInt() ?: 0,
    (get("appointmentsDone") as? Number)?.toInt() ?: 0, (get("presentationsDone") as? Number)?.toInt() ?: 0,
    get("planLocked") as? Boolean ?: false, get("dayLocked") as? Boolean ?: false)
